...
visual_file = "./datasets/BDDX_exp/{}.img.tsv"
# {}.caption.tsv: we use it to store  captions
cap_file = "./datasets/BDDX_exp/{}.caption.tsv"
...
linelist_file = "./datasets/BDDX_exp/{}.linelist.tsv"
...
label_file = "./datasets/BDDX_exp/{}.label.tsv"
...
def dump_tsv_gt_to_coco_format(caption_tsv_file, outfile):
    ...
    for cap_row  in caption_tsv:
        ...
        for i in range(count):
            ...
            caption = action + ' ' + justification
            annotations.append(
                        {'image_id': image_id, 'action': action, 'justification': justification, 'caption': justification,
                        'id': cap_id})
            cap_id += 1
...
def process_new(split):
    ...
    if split == 'training':
        video_list = database['annotations'][:21143]+database['annotations'][26521:27221]
        subtitle = annos[:21143]+annos[26521:27221]
    elif split == 'validation':
        video_list = database['annotations'][21143:23662]+database['annotations'][27221:27321]
        subtitle = annos[21143:23662]+annos[27221:27321]
    elif split == 'testing':
        video_list = database['annotations'][23662:26521]+database['annotations'][27321:]
        subtitle = annos[23662:26521]+annos[27321:]
    else:
        exit("split error")
    asr_len = []
    ...
    for video_ord, video_info in enumerate(video_list):
        ...
        if True:
            if True:
                ...
                resolved_data_vid_id = split + '_' + resolved_video_name + '_' + str(sample_id).zfill(5)
                output_captions = []

                output_captions.append({"action": action, "justification": justification, "caption": justification})
