...
def extract_frame(item, save_dir, fps, num_frames, debug=False, corrupt_files=[]):
    video_file_path1=item['vidName']
    video_file_path="datasets/own/raw_videos/clip2caption/"+video_file_path1+".mp4"
    sTime=item['sTime']
    sTime=int(sTime)
    eTime=item['eTime']
    eTime=int(eTime)
    ...
    if launch_extract:
        ...
        # scale=width:height
        extract_frame_from_video(video_file_path, frame_save_path, fps=fps, num_frames=num_frames,start_ts=sTime, end_ts=eTime,
...
def extract_all_frames(video_root_dir, save_dir, fps, num_frames,
    ...
    raw_video_info = 'datasets/own/captions_ccd.json'
    split = 'training'

    if split == 'training':
        data = json.load(open(raw_video_info))['annotations'][:]
    elif split == 'validation':
        data = json.load(open(raw_video_info))['annotations'][280:290]
    elif split == 'testing':
        data = json.load(open(raw_video_info))['annotations'][290:]
    else:
        exit("split error")
    ...
    if num_workers > 0:
        ...
        with mp.Pool(num_workers) as pool, tqdm(total=len(data)) as pbar:
            for idx, _ in enumerate(
                    pool.imap_unordered(
                        extract_frame_partial, data, chunksize=8)):
                pbar.update(1)
    else:
...
def main():
    ...
    extract_all_frames(args.video_root_dir, args.save_dir, args.fps,
                       ...
                       num_workers=32, debug=args.debug)
