...
def dump_tsv_gt_to_coco_format(caption_tsv_file, outfile):
    ...
    for cap_row  in caption_tsv:
        ...
        for i in range(count):
            ...
            annotations.append(
                        {'image_id': image_id, 'action': action, 'justification': justification, 'caption': caption,
                        'id': cap_id})
            cap_id += 1
    ...
    for video_ord, video_info in enumerate(video_list):
        ...
        if True:
            if True:
                ...
                output_captions.append({"action": action, "justification": justification, "caption": caption})
