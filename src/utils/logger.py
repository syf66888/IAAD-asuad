...
def add_log_to_file(log_path):
    ...
    fh.setFormatter(formatter)
    LOGGER.addHandler(fh)
    #创建一个用于将日志记录写入文件的函数，通过指定日志文件的路径，将日志处理器添加到全局日志记录器中，并设置日志的格式
...
class TensorboardLogger(object):
    ...
    def log_scalar_dict(self, log_dict, prefix=''):
        ...
        if self._logger is None:
            return
        if prefix:
            prefix = f'{prefix}_'#字符串格式化操作，用于在给定的 prefix 变量的末尾添加下划线（_）字符。字符串格式化操作使用 f 字符串（f-string）来构建新的字符串
        for name, value in log_dict.items():
            if isinstance(value, dict):#检查一个对象是否属于指定的类或类型（dict)
