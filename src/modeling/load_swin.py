...
def get_swin_model(args):
    if int(args.img_res) == 384:
        ...
        config_path = 'src/modeling/video_swin/swin_%s_patch244_window877_kinetics%s_1k.py'%(args.vidswin_size, args.kinetics)
        model_path = 'models/video_swin_transformer/swin_%s_patch244_window877_kinetics%s_1k.pth'%(args.vidswin_size, args.kinetics)
        #base,
...
class myVideoSwin(torch.nn.Module):
    def __init__(self, args, cfg, backbone):
        ...
        #确保子类 myVideoSwin 在初始化时会继承父类 torch.nn.Module 的所有属性和方法，并且进行必要的初始化，以确保整个类的正确工作。
